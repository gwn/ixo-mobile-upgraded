...
class Register extends React.Component<Props, StateTypes> {
  ...
  handleUnselectedToSelected(mnemonic: IMnemonic) {
    ...
    const mnemonicWord = mnemonicWords.find(
      (e: IMnemonic) => e.key === mnemonic.key,
    );
    ...
    const mnemonicEntered: string = this.getMnemonicString(
      this.state.selectedWords,
    );
    if (
      this.state.selectedWords.length === this.state.unSelectedWords.length &&
      mnemonicEntered !== this.state.mnemonic
    ) {
  ...
  handleSelectedToUnselected(mnemonic: IMnemonic) {
    ...
    this.setState({
      selectedWords: this.state.selectedWords.filter(
        (e: IMnemonic) => e.key !== mnemonic.key,
      ),
      unSelectedWords: mnemonicWords,
    });
  ...
  handleConfirmMnemonic() {
    ...
    if (
      this.getMnemonicString(this.state.selectedWords) !== this.state.mnemonic
    ) {
      ...
      .then((signature: any) => {
        this.props.ixo.user
          ...
          .then((response: any) => {
            if (response.code === 0) {
              showToast(
                this.props.screenProps.t('register:didLedgeredSuccess'),
                toastType.SUCCESS,
              );
              this.navigateToLogin();
            } else {
              showToast(
                this.props.screenProps.t('register:didLedgeredError'),
                toastType.DANGER,
              );
          ...
          .catch((error) => {
            ...
            showToast(
              this.props.screenProps.t('register:failedToLedgerUser'),
              toastType.DANGER,
            );
            this.setState({ loading: false });
          });
  ...
  handleCreatePassword() {
    if (
      ...
      showToast(
        this.props.screenProps.t('register:missingFields'),
        toastType.WARNING,
      );
      return;
    }
    ...
    if (this.state.password !== this.state.confirmPassword) {
      showToast(
        this.props.screenProps.t('register:missmatchPassword'),
        toastType.WARNING,
      );
      return;
    }
    ...
    if (this.state.password.length < 8) {
      showToast(
        this.props.screenProps.t('register:passwordShort'),
        toastType.WARNING,
      );
      return;
    }
            ...
            <KeyboardAvoidingView behavior={'padding'}>
              <View style={[RegisterStyles.flexLeft]}>
                <Text style={[RegisterStyles.header]}>
                  {this.props.screenProps.t('register:register')}
                </Text>
              </View>
              ...
              <Text
                ...
                {this.props.screenProps.t('register:letsSetup')}
              </Text>
              <InputField
              ...
              <DarkButton
                ...
                text={this.props.screenProps.t('register:create')}
              />
              <TouchableOpacity
                style={{ paddingBottom: 30 }}
                onPress={() => this.props.navigation.navigate('Recover')}>
                <Text style={RegisterStyles.recoverText}>
                  {this.props.screenProps.t('register:recoverAccount')}
                </Text>
              </TouchableOpacity>
            </KeyboardAvoidingView>
          </Content>
      ...
      case registerSteps.revealMnemonic:
        return (
          <View>
            <View style={[RegisterStyles.flexLeft]}>
              <Text style={[RegisterStyles.header]}>
                {this.props.screenProps.t('register:secretPhrase')}
              </Text>
            </View>
            <View style={{ width: '100%' }}>
              <View style={RegisterStyles.divider} />
      ...
      case registerSteps.reenterMnemonic:
        return (
          <View>
            <View style={[RegisterStyles.flexLeft]}>
              <Text style={[RegisterStyles.header]}>
                {this.props.screenProps.t('register:confirmSecret')}
              </Text>
            </View>
            <View style={{ width: '100%' }}>
              <View style={RegisterStyles.divider} />
    ...
    return (
      <View style={[RegisterStyles.selected]}>
        {this.state.selectedWords.map((mnemonic: IMnemonic) => {
          return (
            <TouchableOpacity
              onPress={() => this.handleSelectedToUnselected(mnemonic)}
              key={mnemonic.word}>
              <Text
                style={
                  this.state.errorMismatch
    ...
    return (
      <View
        ...
        {this.state.unSelectedWords.map((mnemonicWord: IMnemonic) => {
          return (
            <TouchableOpacity
              onPress={() => this.handleUnselectedToSelected(mnemonicWord)}
